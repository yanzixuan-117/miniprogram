// pages/admin/coach-manage/coach-edit.js
...
Page({
  ...
  async loadCoachInfo() {
    ...
    try {
      ...
      if (res.result.success) {
        ...
        // 处理头像URL
        let avatarUrl = coach.avatarUrl || '/images/avatar.png'

        // 如果是云存储URL，尝试转换为临时URL
        if (avatarUrl.startsWith('cloud://')) {
          try {
            const processedUrl = await util.processCloudImageURL(avatarUrl)
            // 只有转换成功且不是原云存储URL时才使用
            if (processedUrl && !processedUrl.startsWith('cloud://')) {
              avatarUrl = processedUrl
            } else {
              // 转换失败或仍然是云存储URL，使用默认头像
  ...
  onPriceInput(e) {
    this.setData({
      'formData.price': parseInt(e.detail.value) || 0
    })
  },
